...
create_db<-function(file_layer){
  ...
  # if(grepl("PCN",layer_id)) poly_subset1<-polygons %>% filter(woreda=="North Mecha",kebele=="Enashenefalen")
  # poly_subset<-polygons %>% filter(woreda!="North Mecha",kebele!="Enashenefalen")
  nm<-polygons
  poly_layer<-lapply(layer_star,function(x) st_crop(x,nm))
  poly_layer_sf<-lapply(poly_layer,st_as_sf)
  intersection<-lapply(poly_layer_sf,function(x) st_join(x,nm,join = st_intersects))
