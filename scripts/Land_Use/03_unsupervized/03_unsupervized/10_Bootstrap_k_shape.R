...
pc_clusters<-list()
for(i in 1:bootstrap){
  pc_clusters[[i]]<-theo_cetroids(ndvi_long_split_boot[[i]],semilla=8)
}
...
bag_centroids<-bag_centroids %>% mutate(centroid=factor(centroid))

ggplot(bag_centroids) +
  geom_line(aes(x=date,y=ndvi,col=centroid,group=centroid)) +
   theme_classic()


saveRDS(bag_centroids,here("data/bagging_centroids.Rds"))
