...
c1<-tibble(centroid=1,ts_centroid=c1,dates=colnames(ndvi_long)[3:70])
c2<-tibble(centroid=2,ts_centroid=c2,dates=colnames(ndvi_long)[3:70])
c3<-tibble(centroid=3,ts_centroid=c3,dates=colnames(ndvi_long)[3:70])
c4<-tibble(centroid=4,ts_centroid=c4,dates=colnames(ndvi_long)[3:70])
cent<-rbind(c1,c2,c3,c4)
cent<- cent %>% mutate(dates=lubridate::ymd(dates))
