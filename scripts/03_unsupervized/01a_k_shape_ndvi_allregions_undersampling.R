...
save.image(here("data/results/kshape_undersampling.RData"))
...
clust_opt <- tsclust(ts_ndvi_long_split, k = 13L, distance = "sbd", centroid = "shape", seed = 8, trace = TRUE)

plot(clust_opt,type="centroids")
ggsave("views/centroids_all_regions_undersampling.pdf",height = 6,width=8)

plot(clust_opt,type="series")
ggsave("views/series_all_regions_undersampling.pdf",height = 6,width=8)


ndvi_long_split_labels<- ndvi_long_split_labels %>% mutate(cluster = clust_opt@cluster)

require(xtable)
table<-with(ndvi_long_split_labels,table(land_use,cluster))
table
xtable(table, type = "latex")
print(xtable(table, type = "latex"), file = "views/confusion_undersampling.tex")
?print.xtable
